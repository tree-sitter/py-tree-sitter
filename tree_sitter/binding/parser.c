...
static bool progress_callback(TSParseState *state) {
    PyObject *byte_offset = PyLong_FromUnsignedLong(state->current_byte_offset);
    PyObject *has_error = state->has_error ? Py_True : Py_False;
    PyObject *args = PyTuple_Pack(2, byte_offset, has_error);
    Py_XDECREF(byte_offset);

    // Call callable.
    PyObject *result = PyObject_Call((PyObject *)state->payload, args, NULL);
    Py_XDECREF(args);
    return result;
}
